...
export const Drone = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    if (actions && actions["hover"]) {
      actions["hover"].reset().fadeIn(0.5).play(); // Inicializa la animación
    }

    return () => {
      if (actions && actions["hover"]) {
        actions["hover"].fadeOut(0.5); // Detén la animación cuando se desmonte el componente
      }
    };
  }, [actions]);
...
useGLTF.preload("/models/drone-model.glb");
